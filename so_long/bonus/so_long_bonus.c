...
/*   Updated: 2025/01/10 11:10:48 by eel-garo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char *av[])
{
    ...
    data.win = mlx_new_window(data.mlx, (data.y * IMAGE_SIZE), (data.x * IMAGE_SIZE), "so_long");
    if (!data.win)
        return (ft_cleanup_game(&data), EXIT_FAILURE);
    ft_get_img_data(&data);
    ft_drow_map(&data);
    mlx_hook(data.win, 2, 1 << 0, ft_first_move, &data); // Register key hook
    mlx_loop_hook(data.mlx, game_loop, &data); // Register game loop
    mlx_hook(data.win, 17, 0, ft_exit, &data); // Register exit hook
    mlx_loop(data.mlx);
    return (0);
}
